...
`vista-gfx-scene-flow` is centered on graphics work in Dart. The technical target is: Design a Dart verification harness for scene systems, covering simulation kernel, seeded input scenarios, and failure-oriented tests.
...
The score is deterministic. The current threshold is 169. Risk has a penalty of 5, latency has a penalty of 4, and weight contributes a bonus of 3.
